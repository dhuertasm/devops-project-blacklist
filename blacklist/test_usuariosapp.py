...
def test_ping():
    response = app.test_client().get('/users/ping')
...
def test_user_singin_201_412():
    ...
    response_data_201= app.test_client().post('/users', json=data)
    response_info_201 = json.loads(response_data_201.data.decode('utf-8'))

    response_data_412 = app.test_client().post('/users', json=data)
...
def test_user_singin_400():
    ...
    response_data = app.test_client().post('/users', json=data)
...
def test_user_login_200_400():
    ...
    app.test_client().post('/users', json=data)

    data_login = {"username": data['username'],
                  "password": data['password'],
                  }

    response_data_200 = app.test_client().post('/users/auth', json=data_login)
    response_info_200 = json.loads(response_data_200.data.decode('utf-8'))

    data_login_miss = {"username": data['username']
                  }

    response_data_400 = app.test_client().post('/users/auth', json=data_login_miss)
...
def test_user_singin_404():
    ...
    response_data_404 = app.test_client().post('/users/auth', json=data)
...
def test_user_me_200_401():
    ...
    response_data = app.test_client().post('/users/auth', json=data_login)
    response_info = json.loads(response_data.data.decode('utf-8'))

    response_data_200 = app.test_client().get("/users/me", headers={"Authorization": "Basic {}".format(response_info['token'])})
    response_info_200 = json.loads(response_data_200.data.decode('utf-8'))
...
def test_user_me_400():
    ...
    response_data_400 = app.test_client().get("/users/me")
    response_info_400 = json.loads(response_data_400.data.decode('utf-8'))
...
def test_user_me_401():
    ...
    response_data_401 = app.test_client().get("/users/me", headers={"Authorization": "Basic token"})
